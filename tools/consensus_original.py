...
CONSENSUS_WORKFLOW_FIELD_DESCRIPTIONS = {
    ...
    "models": (
        "List of model configurations to consult. Can be left empty or set to ['auto'] for automatic model selection. "
        "For manual selection, each entry can have a model name, stance (for/against/neutral), "
        ...
        "Auto mode example: [] or ['auto'] - will select 3-5 diverse models with balanced stances."
    ),
    "current_model_index": (
...
class ConsensusRequest(WorkflowRequest):
    ...
    def validate_step_one_requirements(self):
        """Ensure step 1 has valid models field (can be empty for auto mode) and unique model+stance combinations."""
        if self.step_number == 1:
            # Allow empty models field or ["auto"] for auto mode
            if self.models and not (len(self.models) == 1 and self.models[0] in ["auto", "AUTO"]):
                # Check for unique model + stance combinations only if models are explicitly provided
...
class ConsensusTool(WorkflowTool):
    ...
    def _build_auto_panel(self, topic: str, max_models: int = None) -> list[dict]:
        """
        Build an automatic panel of models for consensus gathering.

        Args:
            topic: The topic/question being discussed (for context)
            max_models: Maximum number of models to include (default: 5, or from MCP_CONSENSUS_MAX_MODELS env var)
        ...
        panel_info = [f"{m['model']}:{m['stance']}" for m in model_configs]
        logger.info(f"Auto-selected consensus panel for '{topic[:50]}...': {panel_info}")

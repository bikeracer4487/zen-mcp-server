...
Write-ColorText "=== Deploying Doug-Zen MCP Server ===" -Color Green
...
function Wait-ForHealth {
    ...
    while ($attempt -le $MaxAttempts) {
        try {
            # Get container ID for doug-zen-mcp service
            $containerId = docker-compose ps -q doug-zen-mcp
...
# Start the services
Write-ColorText "Starting Doug-Zen MCP Server..." -Color Green
try {
    docker-compose up -d
    if ($LASTEXITCODE -ne 0) {
...
Write-ColorText "✓ Doug-Zen MCP Server deployed successfully" -Color Green
...
Write-ColorText "docker-compose logs -f doug-zen-mcp" -Color Green
...
Write-ColorText "docker-compose restart doug-zen-mcp" -Color Green
